...
## 2024-08-08 수정 내용 
9050s, 1080i 모델 통합 펌웨어 최초 업로드

### LGAom 
- 기타 설정 메뉴에서 모델 버전 설정 기능 추가
- 버전이 바뀔 때 마다 기타 설정 메뉴에서 버튼 배열 선택 항목 텍스트 변경 (ex 4x8 3페이지 -> 4x9 4페이지)
- 프로비저닝시 선택된 버전과 버튼 배열에 맞는 페이지 할당
##### (전화기에 "DSS 정보가 변경 되었습니다.", "DSS가 연결 되었습니다." 등의 DSS 관련 문구가 출력되어야 정상 적용. 그렇지 않을 경우 발신이 안 되며 전화기를 재부팅하면 해결 됨)

### LGSettings 
- 설정 메인 화면에 현재 버전 명시
- 시스템 설정 메뉴에서 모델 버전 설정 기능 추가
